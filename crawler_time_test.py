...
if __name__ == '__main__':
    ...
    t2 = time.time()
    print("Time to count for query {}: {} seconds.".format(query, t2-t1))

    done_articles = 0

    while remaining_articles > 0:
        print("Fetching next {}...".format(batch_size))
        t1 = time.time()
        cursor = articles.find(query, ["_id"], skip = done_articles, limit=batch_size, no_cursor_timeout=True)
        t2 = time.time()
        print("Time to create cursor for query {}: {} seconds.".format(query, t2-t1))
        ...
        #     queue.put('STOP')
        # t2 = time.time()
        # print("Time to load queue: {} seconds.".format(t2-t1))
        # # del(queue)
        ...
        # t2 = time.time()
        # print("Time to count for query {} was {} seconds.".format(query, t2-t1))

        # remaining_articles = articles.count(query)
        remaining_articles = remaining_articles - batch_size
        done_articles = done_articles + batch_size
        print("Done articles: {}. Remaining articles: {}.".format(done_articles, remaining_articles))
